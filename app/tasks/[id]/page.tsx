...
export default function TaskDetail({ params }: { params: { id: string } }) {
  ...
  if (!task) {
    return (
      <main className="min-h-screen bg-slate-100 dark:bg-slate-900 p-4">
        <div className="mx-auto max-w-2xl rounded-xl bg-white dark:bg-slate-800 p-6">
  ...
  return (
    <main className="min-h-screen bg-slate-100 dark:bg-slate-900 p-4">
      ...
      */}
      <div className="mx-auto max-w-2xl rounded-xl bg-white dark:bg-slate-800 p-6 shadow-sm">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {task.reqskills.map((skill: string, index: number) => (
              <span
                key={index}
                className="rounded-full bg-slate-100 dark:bg-slate-700 px-3 py-1 text-sm text-slate-700 dark:text-slate-200"
              >
                {skill}
              </span>
            ))}
